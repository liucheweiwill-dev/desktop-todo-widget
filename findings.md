...
## D8. 現成工具能否取代本專案（2026-09-09 收尾時提出）

使用者在專案收尾時問：「是不是有 Google 服務或其他 App 可以達成這個 widget 八成以上的功能？」

**會，而且最接近的那個已經內建在端機上：Windows 內建「便利貼」（Sticky Notes）。**

| 功能 | 本專案 | Windows 便利貼 | Google Keep |
|---|---|---|---|
| 新增／勾選／編輯／刪除 | 有 | 有 | 有 |
| 拖曳排序 | 有 | 無 | 有 |
| **每列**字型大小 | **有** | 無 | 無 |
| **每列**顏色 | **有** | 僅整張換色 | 僅整張換色 |
| 常駐桌面可見 | 會被蓋掉 | 會被蓋掉 | 需開瀏覽器 |
| 完全離線、無帳號 | **有** | 可（不登入） | 需帳號 |
| **端機安裝成本** | 傳 60MB＋過防毒 | **零，已內建** | 只需瀏覽器 |

便利貼的**可見性特徵與本專案幾乎相同**——同樣是普通視窗、同樣會被最大化視窗蓋掉、
同樣被 Win+D 藏起來。而它不需要散布，因此完全不會遇到 D5／防毒那一整串問題。

### 本專案僅存的不可取代之處

1. **每列獨立的字型大小與顏色**（U2）——便利貼與 Keep 都只能整張筆記換色
2. **保證離線、不需任何帳號**

除此之外沒有了。

### 這個檢查應該排在什麼時候（本專案的流程缺口）

`task_plan.md` 把 S1 排在所有功能之前，理由是「先驗證最可能推翻整個設計的假設」。
這個安排是對的，**但 S1 驗證的是「做不做得到」，不是「該不該做」。**

S1 的任務卡自己寫了：「**S1 失敗不代表專案失敗，代表產品定義要調整。**」
S1 確實失敗了（`workerw` 不可交付，桌面嵌入無公開 API），產品定義也確實調整了
（改為「bottommost 常駐視窗」）。

**但沒有人在那個時間點回頭問：調整後的產品定義，是否還贏得過現成工具？**

原始定義「嵌進桌面圖示層、像真 widget 一樣」確實沒有現成替代品——那是它值得自建的理由。
D2 證明那做不到之後，剩下的是「一個會被蓋掉的無邊框視窗」，
而那正好是便利貼二十年來一直在做的事。

**流程上的教訓：當 spike 的結論是「產品定義要調整」時，
調整後的定義必須重新跟現成方案比一次，而不是直接往下做功能。**
本專案在 S1 之後直接進入 W1，這一步從未執行。
